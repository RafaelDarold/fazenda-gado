...
interface Animal { id: string; brinco: string; nome: string | null; categoria: string; lote_id: string | null }
interface Lote    { id: string; nome: string; quantidade_atual: number }
interface Pesagem {
  id: string; data: string; peso_arroba: string; peso_kg: string
  gmd_arroba: string | null; animal_brinco: string; animal_categoria: string
  responsavel: string | null
}

export async function pesagensPage() {
  renderLayout('Pesagens', '<div class="loading"><div class="spinner"></div>Carregando...</div>')
  ...
  try {
    ...
    document.getElementById('app')!.innerHTML = `
      <div class="page-header">
        ...
        <div style="display:flex;gap:var(--sp-3)">
          <button class="btn btn--fantasma" id="btn-pesagem-individual">⚖ Individual</button>
          <button class="btn btn--primario"  id="btn-pesagem-lote">⚖ Pesagem em Lote</button>
        </div>
      </div>
    ...
    document.getElementById('form-pesagem-individual')?.addEventListener('submit', async (e) => {
      e.preventDefault()
      const form = e.target as HTMLFormElement
      const data = Object.fromEntries(new FormData(form))
      try {
        const res = await api.post<{ data: { peso_arroba: string; peso_kg: string; gmd_arroba: string | null } }>('/pesagens', {
        ...
        form.reset()
        ;(form.querySelector('[name="data"]') as HTMLInputElement).value = hoje()
        ...
        `
        ;(document.getElementById('modal-resultado') as HTMLElement).style.display = 'flex'
        carregarHistorico()
      } catch (err) {
    ...
    document.getElementById('btn-carregar-animais-lote')?.addEventListener('click', async () => {
      const loteId = (document.getElementById('sel-lote-pesagem') as HTMLSelectElement).value
      if (!loteId) { toast.warning('Selecione um lote primeiro'); return }

      try {
        const res = await api.get<{ data: Animal[] }>(`/lotes/${loteId}/animais`)
        ...
        const linhas = animaisLote.map(a => `
          <tr>
        ...
        document.getElementById('animais-lote-lista')!.innerHTML = `
          <div class="table-wrap">
            <table>
              <thead><tr><th>Brinco / Nome</th><th>Categoria</th><th>Peso (@) *</th></tr></thead>
              <tbody>${linhas}</tbody>
            </table>
          </div>
          <p class="text-muted text-sm mt-4">Deixe o campo em branco para pular um animal.</p>
        `
        ;(document.getElementById('tabela-lote-pesagem') as HTMLElement).style.display = 'block'
      } catch (err) {
        toast.error(err instanceof Error ? err.message : 'Erro ao carregar animais')
      }
    })

    document.getElementById('btn-salvar-pesagem-lote')?.addEventListener('click', async () => {
      const form      = document.getElementById('form-pesagem-lote-select') as HTMLFormElement
      const formData  = Object.fromEntries(new FormData(form))
      const loteId    = (document.getElementById('sel-lote-pesagem') as HTMLSelectElement).value
      const inputs    = document.querySelectorAll<HTMLInputElement>('.peso-lote-input')

      const pesagens: Array<{ animal_id: string; peso_arroba: number }> = []
      inputs.forEach(input => {
        if (input.value) {
          pesagens.push({ animal_id: input.dataset.animalId!, peso_arroba: parseFloat(input.value) })
        }
      })

      if (pesagens.length === 0) { toast.warning('Informe o peso de pelo menos um animal'); return }

      try {
        const res = await api.post<{ data: { registradas: number; ignoradas: number; pesoMedioLote: number } }>('/pesagens/lote', {
          lote_id:     loteId,
          data:        formData.data,
        ...
        const r = res.data
        toast.success(`${r.registradas} pesagem(ns) registrada(s)! Peso médio do lote: ${formatArroba(r.pesoMedioLote)}`)
        ;(document.getElementById('tabela-lote-pesagem') as HTMLElement).style.display = 'none'
        form.reset()
        ;(form.querySelector('[name="data"]') as HTMLInputElement).value = hoje()
        carregarHistorico()
      } catch (err) {
        toast.error(err instanceof Error ? err.message : 'Erro ao salvar pesagens')
      }
    })

    document.getElementById('btn-fechar-resultado')?.addEventListener('click', () => {
      ;(document.getElementById('modal-resultado') as HTMLElement).style.display = 'none'
    })

    carregarHistorico()

  } catch (err) {
    document.getElementById('app')!.innerHTML = `<div class="empty-state"><h3>Erro ao carregar pesagens</h3></div>`
  }
}
...
async function carregarHistorico() {
  ...
  try {
    ...
    const res = await api.get<{ data: Pesagem[] }>('/pesagens/lote/all?pageSize=30').catch(() => ({ data: [] }))
    ...
    const linhas = res.data.map((p: Pesagem) => `
      <tr>
