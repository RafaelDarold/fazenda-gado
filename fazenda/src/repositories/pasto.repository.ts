...
export class PastoRepository extends BaseRepository<Pasto> {
  ...
  async findAll(apenasAtivos = true): Promise<Pasto[]> {
    const where = apenasAtivos ? 'WHERE ativo = 1' : ''
    return query<Pasto>(`SELECT * FROM pasto ${where} ORDER BY nome`)
  }
   ...
   */
  async findComOcupacao(): Promise<PastoComOcupacao[]> {
  ...
  async create(dto: CreatePastoDTO): Promise<Pasto> {
    const id = uuid()
    await execute(
      `INSERT INTO pasto (id, nome, area_hectares, tipo_capim, capacidade_ua, observacao)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        id,
        dto.nome,
  ...
  async update(id: UUID, dto: UpdatePastoDTO): Promise<Pasto | null> {
    ...
    if (dto.area_hectares !== undefined) { fields.push('area_hectares = ?');  values.push(dto.area_hectares) }
    if (dto.tipo_capim    !== undefined) { fields.push('tipo_capim = ?');     values.push(dto.tipo_capim) }
    if (dto.capacidade_ua !== undefined) { fields.push('capacidade_ua = ?');  values.push(dto.capacidade_ua) }
