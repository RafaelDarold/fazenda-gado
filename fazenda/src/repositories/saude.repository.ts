...
export class SaudeEventoRepository extends BaseRepository<SaudeEvento> {
  ...
  async findAll(filtro: FiltroSaudeEvento = {}): Promise<SaudeEventoDetalhado[]> {
    const conditions: string[] = []
    const values: unknown[] = []

    if (filtro.tipo)    { conditions.push('se.tipo = ?');    values.push(filtro.tipo) }
    if (filtro.escopo)  { conditions.push('se.escopo = ?');  values.push(filtro.escopo) }
    if (filtro.lote_id) { conditions.push('se.lote_id = ?'); values.push(filtro.lote_id) }
    if (filtro.data_inicio) { conditions.push('se.data_aplicacao >= ?'); values.push(filtro.data_inicio) }
    if (filtro.data_fim)    { conditions.push('se.data_aplicacao <= ?'); values.push(filtro.data_fim) }
    if (filtro.proximos_dias !== undefined) {
      conditions.push('se.data_proxima BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL ? DAY)')
  ...
  async createEvento(
    ...
    const sql = `
      INSERT INTO saude_evento
        (id, escopo, lote_id, tipo, produto, fabricante, lote_produto,
  ...
  async vincularAnimal(
    ...
    const sql = `
      ...
      VALUES (?, ?, ?, ?, ?)
    `
    const params = [id, eventoId, animalId, doseAplicadaMl ?? null, observacaoIndividual ?? null]
    if (conn) { await conn.execute(sql, params) } else { await execute(sql, params) }
  }

  /** Vincula todos os animais ativos de um lote a um evento */
  async vincularLote(eventoId: UUID, loteId: UUID, conn?: PoolConnection): Promise<number> {
    const animais = await query<{ id: UUID }>(
      'SELECT id FROM animal WHERE lote_id = ? AND ativo = 1',
      [loteId],
    )
    for (const animal of animais) {
      await this.vincularAnimal(eventoId, animal.id, undefined, undefined, conn)
    }
