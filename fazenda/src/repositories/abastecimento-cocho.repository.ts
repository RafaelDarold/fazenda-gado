...
export class AbastecimentoCochoRepository extends BaseRepository<AbastecimentoCocho> {
  ...
  async findByPasto(pastoId: UUID, dataInicio?: DateString, dataFim?: DateString): Promise<AbastecimentoCochoDetalhado[]> {
    const conditions = ['ac.pasto_id = ?']
    const values: unknown[] = [pastoId]

    if (dataInicio) { conditions.push('ac.data >= ?'); values.push(dataInicio) }
    if (dataFim)    { conditions.push('ac.data <= ?'); values.push(dataFim) }
  ...
  async findAll(dataInicio?: DateString, dataFim?: DateString): Promise<AbastecimentoCochoDetalhado[]> {
  ...
  async create(dto: CreateAbastecimentoCochoDTO, lancamentoId?: UUID, conn?: PoolConnection): Promise<AbastecimentoCocho> {
    const id = uuid()
    const sql = `
      INSERT INTO abastecimento_cocho
        (id, pasto_id, data, tipo, quantidade_kg, custo_total,
         fornecedor, lancamento_financeiro_id, observacao)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `
    const params = [
      id, dto.pasto_id, dto.data, dto.tipo, dto.quantidade_kg,
      dto.custo_total ?? null, dto.fornecedor ?? null,
      lancamentoId ?? null, dto.observacao ?? null,
    ]
