...
export class FinanceiroService {
  ...
  async listarCategorias(tipo?: TipoLancamento): Promise<CategoriaFinanceira[]> {
  ...
  async pendentes(): Promise<LancamentoDetalhado[]> {
    return lancamentoFinanceiroRepository.findPendentes()
  }
  ...
  async lancar(dto: CreateLancamentoDTO): Promise<LancamentoFinanceiro> {
    const categoria = await categoriaFinanceiraRepository.findById(dto.categoria_id)
    if (!categoria) throw new Error(`Categoria não encontrada: ${dto.categoria_id}`)
  ...
  async cancelar(id: UUID): Promise<void> {
    const lanc = await lancamentoFinanceiroRepository.findById(id)
    if (!lanc) throw new Error(`Lançamento não encontrado: ${id}`)
    if (lanc.status === 'cancelado') throw new Error('Lançamento já está cancelado')
    await lancamentoFinanceiroRepository.cancelar(id)
  }

  // ── Resumo para dashboard ───────────────────────────────────────────────────

  async resumoPeriodo(dataInicio: DateString, dataFim: DateString): Promise<{
    ...
    const [resumo, pendentes] = await Promise.all([
      lancamentoFinanceiroRepository.resumoPeriodo(dataInicio, dataFim),
