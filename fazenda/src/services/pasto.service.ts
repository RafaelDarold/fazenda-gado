...
export class PastoService {
  ...
  async listar(fazendaId?: string): Promise<PastoComOcupacao[]> {
    return pastoRepository.findComOcupacao(fazendaId);
  }
  ...
  async moverLote(
    ...
    if (!novoPasto)
      throw new Error(`Pasto destino não encontrado: ${novoPastoId}`);

    await transaction(async (conn) => {
      // Encerra ocupação atual se existir
      if (lote.pasto_atual_id) {
        const ocAtual = await ocupacaoPastoRepository.findAtualByPasto(
          lote.pasto_atual_id,
        );
        if (ocAtual) {
          await ocupacaoPastoRepository.encerrar(
            ocAtual.id,
            { data_saida: data, observacao },
            conn,
          );
        }
      }
      ...
      await loteRepository.update(
        loteId,
        { pasto_atual_id: novoPastoId },
        conn,
      );
    });
  }
  ...
  async abrirOcupacao(
    dto: CreateOcupacaoPastoDTO,
  ): Promise<OcupacaoPastoDetalhada> {
  ...
  async historicoCocho(
    ...
    return abastecimentoCochoRepository.findByPasto(
      pastoId,
      dataInicio,
      dataFim,
    );
